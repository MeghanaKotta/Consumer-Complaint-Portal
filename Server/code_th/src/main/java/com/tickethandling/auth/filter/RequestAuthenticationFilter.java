package com.tickethandling.auth.filter;
...
public class RequestAuthenticationFilter implements Filter {
	...
	protected static final List<String> ALLOWED_URL_LIST = Arrays.asList(" user/register", "/quoraStudent/user/login",
			"/quoraStudent/user/checksession", "/quoraStudent/info/getUnvList", "/quoraStudent/info/getInterests",
			"/quoraStudent/account/activateEmail");
...
//		HttpSession session = request.getSession(false);
		...
		if (ALLOWED_URL_LIST.contains(url) || url.endsWith(".css") || url.endsWith(".js") || url.endsWith(".png")
			...
			try {
				boolean loggedInStatus = (boolean) userService.checkSession(sessionkey == null ? "" : sessionkey)
						.get("valid");
				if (ObjectUtils.isEmpty(sessionkey) || !loggedInStatus) {
